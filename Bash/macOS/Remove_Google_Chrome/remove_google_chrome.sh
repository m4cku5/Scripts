...
## Removes the Google Chrome app for macOS.

set -eu -o pipefail

# User-defined variables
dirs=("/Applications/Google Chrome.app" "/Library/Application Support/Google/Chrome")
scriptName="remove_google_chrome_sh"
...
function is_dir() {
    printf "$(date) | Checking if directory exists.\n"
    if [ -d "$dir" ]; then
